...
OUT_ROOT="${REPO_ROOT}/output/ablation/softoverlap_gate090_paired_v1"
PORT="${IMED_PORT:-6024}"
SEED="${IMED_SEED:-1}"
...
if ss -ltnH "sport = :${PORT}" | grep -q .; then
    echo "Port ${PORT} is occupied. Retry with: IMED_PORT=6025 $0"
    exit 2
fi
...
"${PYTHON_BIN}" scripts/compare_imed_soft_overlap_gate.py
echo "All paired soft-overlap gate experiments completed."

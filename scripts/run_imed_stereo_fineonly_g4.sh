...
OUT_ROOT="${REPO_ROOT}/output/ablation/stereo_fineonly_g4"
CONFIG="arguments/imed_extent10_principal_stereo_fineonly_g4_iter2500.py"
PORT="${IMED_PORT:-6044}"
SEED="${IMED_SEED:-1}"
...
wait_for_gpu0() {
    ...
    while true; do
        ...
        if (( gpu_util <= 10 && gpu_mem <= 1000 )); then return 0; fi
        echo "GPU0 busy: utilization=${gpu_util}%, memory=${gpu_mem} MiB; retrying in 30 s."
        sleep 30
    done
}

if [[ ! -x "${PYTHON_BIN}" ]]; then echo "Python not found: ${PYTHON_BIN}"; exit 1; fi
for seq in "${seqs[@]}"; do
    for control in \
        "output/ablation/stereo_principal_v2/baseline_pp/$seq/results.json" \
        "output/ablation/stereo_principal_v2/g2v2_rgb/$seq/results.json" \
        "output/ablation/stereo_photogate_g3/$seq/results.json"
    do
        if [[ ! -s "${control}" ]]; then echo "Missing paired control: ${control}"; exit 2; fi
    done
done
...
for seq in "${seqs[@]}"; do
    output="${OUT_ROOT}/${seq}"
    log="${OUT_ROOT}/${seq}.log"
    if [[ -s "${output}/results.json" ]]; then
        echo "Complete result exists, skipping: ${seq}"
        continue
    fi
    if [[ -e "${output}" ]]; then
    ...
    status=${PIPESTATUS[0]}; (( status == 0 )) || exit "${status}"
    ...
    CUDA_VISIBLE_DEVICES=0 "${PYTHON_BIN}" metrics.py -m "${output}" \
        2>&1 | tee -a "${log}"
    status=${PIPESTATUS[0]}; (( status == 0 )) || exit "${status}"
done

"${PYTHON_BIN}" scripts/compare_imed_stereo_fineonly_g4.py
echo "Fine-stage-only stereo G4 pilot completed."

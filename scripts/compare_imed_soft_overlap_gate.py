...
ROOT = Path("output/ablation/softoverlap_gate090_paired_v1")
SEQUENCES = (
...
print("\n===== Two-sequence equal-weight mean delta =====")
for metric in METRICS:
    print(f"{metric:5s}: mean_delta={sum(deltas[metric]) / len(deltas[metric]):+.8f}")

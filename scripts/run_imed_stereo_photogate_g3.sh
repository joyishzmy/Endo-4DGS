...
CONTROL_ROOT="${REPO_ROOT}/output/ablation/stereo_principal_v2"
OUT_ROOT="${REPO_ROOT}/output/ablation/stereo_photogate_g3"
CONFIG="arguments/imed_extent10_principal_stereo_photogate_g3_iter2500.py"
PORT="${IMED_PORT:-6043}"
SEED="${IMED_SEED:-1}"
...
for seq in "${seqs[@]}"; do
    for method in baseline_pp g2v2_rgb; do
        if [[ ! -s "${CONTROL_ROOT}/${method}/${seq}/results.json" ]]; then
            echo "Missing paired control: ${CONTROL_ROOT}/${method}/${seq}/results.json"
            exit 2
        fi
    done
done
...
"${PYTHON_BIN}" scripts/compare_imed_stereo_photogate_g3.py
echo "Photometric-gated stereo G3 pilot completed."

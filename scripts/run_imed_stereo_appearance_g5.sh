#!/usr/bin/env bash

set -uo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
PYTHON_BIN="${IMED_PYTHON_BIN:-/home/login/Documents/ZMY/conda-envs/imed4dgs/bin/python}"
DATA_ROOT="${IMED_DATA_ROOT:-${REPO_ROOT}/data/imed}"
OUT_ROOT="${REPO_ROOT}/output/ablation/stereo_appearance_g5"
CONFIG="arguments/imed_extent10_principal_stereo_appearance_g5_iter2500.py"
PORT="${IMED_PORT:-6045}"
SEED="${IMED_SEED:-1}"
seqs=(session_004_scene_6_tool_3 session_007_scene_5_tool_2)

cd "${REPO_ROOT}" || exit 1

wait_for_gpu0() {
    local gpu_util gpu_mem
    while true; do
        gpu_util=$(nvidia-smi --id=0 --query-gpu=utilization.gpu --format=csv,noheader,nounits | tr -d ' ')
        gpu_mem=$(nvidia-smi --id=0 --query-gpu=memory.used --format=csv,noheader,nounits | tr -d ' ')
        if (( gpu_util <= 10 && gpu_mem <= 1000 )); then return 0; fi
        echo "GPU0 busy: utilization=${gpu_util}%, memory=${gpu_mem} MiB; retrying in 30 s."
        sleep 30
    done
}

if [[ ! -x "${PYTHON_BIN}" ]]; then echo "Python not found: ${PYTHON_BIN}"; exit 1; fi
for seq in "${seqs[@]}"; do
    for control in \
        "output/ablation/stereo_principal_v2/baseline_pp/$seq/results.json" \
        "output/ablation/stereo_photogate_g3/$seq/results.json" \
        "output/ablation/stereo_fineonly_g4/$seq/results.json"
    do
        if [[ ! -s "${control}" ]]; then echo "Missing paired control: ${control}"; exit 2; fi
    done
done

"${PYTHON_BIN}" scripts/test_imed_stereo_geometry.py || exit $?
"${PYTHON_BIN}" scripts/test_imed_principal_projection.py || exit $?
mkdir -p "${OUT_ROOT}"

for seq in "${seqs[@]}"; do
    output="${OUT_ROOT}/${seq}"
    log="${OUT_ROOT}/${seq}.log"
    if [[ -s "${output}/results.json" ]]; then
        echo "Complete result exists, skipping: ${seq}"
        continue
    fi
    if [[ -e "${output}" ]]; then
        echo "Incomplete output exists; preserving it and stopping: ${output}"
        exit 3
    fi
    wait_for_gpu0
    CUDA_VISIBLE_DEVICES=0 "${PYTHON_BIN}" train.py \
        -s "${DATA_ROOT}/${seq}" -m "${output}" \
        --configs "${CONFIG}" --seed "${SEED}" --port "${PORT}" \
        --save_iterations 2000 2500 2>&1 | tee "${log}"
    status=${PIPESTATUS[0]}; (( status == 0 )) || exit "${status}"

    CUDA_VISIBLE_DEVICES=0 "${PYTHON_BIN}" render.py \
        -m "${output}" --iteration 2500 --skip_train --skip_video \
        2>&1 | tee -a "${log}"
    status=${PIPESTATUS[0]}; (( status == 0 )) || exit "${status}"

    CUDA_VISIBLE_DEVICES=0 "${PYTHON_BIN}" metrics.py -m "${output}" \
        2>&1 | tee -a "${log}"
    status=${PIPESTATUS[0]}; (( status == 0 )) || exit "${status}"
done

"${PYTHON_BIN}" scripts/compare_imed_stereo_appearance_g5.py
echo "Appearance-only stereo G5 pilot completed."
